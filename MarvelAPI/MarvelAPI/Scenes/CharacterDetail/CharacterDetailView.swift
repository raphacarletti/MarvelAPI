...
final class CharacterDetailView: UIView, CharacterDetailViewProtocol {
    ...
    private var mostExpensiveMagazineButton: UIButton = {
        let button = UIButton()
    ...
    func setup() {
        ...
        descriptionLabel.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector(didTapDecriptionLabel)))
        mostExpensiveMagazineButton.addTarget(self, action: #selector(didTapMostExpensiveMagazineButton), for: .touchUpInside)

        addSubview(characterAvatarImageView)
        constrain(self, characterAvatarImageView) { superView, imageView in
        ...
        constrain(self, characterAvatarImageView, stackView) { superView, imageView, stackView in
            stackView.top == imageView.bottom + 16
            stackView.leading == superView.leading + 16
            stackView.trailing == superView.trailing - 16
        }

        addSubview(mostExpensiveMagazineButton)
        constrain(self, stackView, mostExpensiveMagazineButton) { superView, stackView, button in
        ...
        constrain(self, mostExpensiveMagazineButton, activityIndicator) { superView, button, indicator in
